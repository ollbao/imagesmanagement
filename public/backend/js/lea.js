layui.define(['layer', 'form', 'laydate', 'element'], function(exports) {
    ...
    $(document).on('click', '.layui-laypage>a', function() {
        ...
        $('.data-list').data('page', page).getList();
        return false;
    });
    //分页跳转
    ...
    //分页数量
    $(document).on('change', '.layui-laypage>.layui-laypage-limits select', function() {
        var limit = $(this).val() || 20;
        layui.data('leacmf', {
            key: 'limit',
            value: limit
        });
        $('.data-list').data('limit', limit).getList();
        return;
    });
